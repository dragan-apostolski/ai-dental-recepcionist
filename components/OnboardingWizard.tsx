    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 md:p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-3xl relative z-10">
        <div className="text-center mb-6 space-y-2">
          ...
          <h1 className="text-2xl font-black text-white tracking-tight uppercase italic">{t.setupTitle}</h1>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden min-h-[600px] flex flex-col">
          <div className="flex-1 p-8 md:p-10 overflow-y-auto no-scrollbar max-h-[75vh]">
            {step === 1 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-1">
                  <h2 className="text-xl font-black text-slate-900 flex items-center gap-3"><Building2 className="text-teal-600" /> {t.clinic}</h2>
                  <p className="text-xs text-slate-500 font-medium tracking-tight">{t.welcomeSub}</p>
                </div>
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">{t.languageUI}</label>
                    <div className="flex gap-2">
                      {['mk', 'en', 'sl'].map(lang => (
                        <button key={lang} onClick={() => update('uiLanguage', lang)} className={`flex-1 py-3 rounded-xl border font-black text-xs uppercase transition-all ${settings.uiLanguage === lang ? 'bg-teal-600 text-white border-teal-600' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">{t.businessName}</label>
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-1"><h2 className="text-xl font-black text-slate-900 flex items-center gap-3"><Calendar className="text-teal-600" /> {t.calendar}</h2><p className="text-xs text-slate-500 font-medium tracking-tight">{t.calendarSub}</p></div>
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Calendly Token</label>
                    <div className="flex gap-2">
                      ...
                      <button onClick={fetchEvents} disabled={isFetchingEvents || !settings.calendlyToken} className="bg-slate-900 text-white px-6 rounded-2xl font-black text-[9px] uppercase hover:bg-black transition-all disabled:opacity-50">{isFetchingEvents ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />} {t.sync}</button>
                    </div>
                  </div>
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-1"><h2 className="text-xl font-black text-slate-900 flex items-center gap-3"><Sparkles className="text-teal-600" /> {t.services}</h2><p className="text-xs text-slate-500 font-medium tracking-tight">{t.serviceSub}</p></div>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {settings.eventTypes.map(et => (
                      <button key={et.id} disabled={settings.services.some(s => s.calendlyEventTypeUri === et.uri)} onClick={() => addService(et)} className="p-3 rounded-xl border text-[10px] font-black uppercase text-slate-700 hover:border-teal-500 disabled:opacity-30 transition-all">+ {et.title}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {step === 4 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-1"><h2 className="text-xl font-black text-slate-900 flex items-center gap-3"><Clock className="text-teal-600" /> {t.workingHours}</h2><p className="text-xs text-slate-500 font-medium tracking-tight">{t.hoursSub}</p></div>
                <div className="space-y-4">
                  <div className="p-5 bg-teal-50 border border-teal-100 rounded-[2rem] space-y-3">
                    ...
                    <div className="flex gap-2"><button onClick={() => applyToWeekdays({ day: '', open: (document.getElementById('bulk-open') as any).value, close: (document.getElementById('bulk-close') as any).value, isClosed: false })} className="flex-1 py-2 bg-teal-600 text-white rounded-xl text-[9px] font-black uppercase">{t.applyWeekdays}</button><button onClick={() => applyToAllDays({ day: '', open: (document.getElementById('bulk-open') as any).value, close: (document.getElementById('bulk-close') as any).value, isClosed: false })} className="flex-1 py-2 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase">{t.applyAll}</button></div>
                  </div>
                  <div className="space-y-2">
                    {DAYS_ORDER.map(day => {
                      const h = settings.workingHours.find(d => d.day === day)!;
                      return (
                        <div key={day} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl text-[10px] font-black">
            ...
            {step === 5 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-1"><h2 className="text-xl font-black text-slate-900 flex items-center gap-3"><UserCircle className="text-teal-600" /> {t.persona}</h2><p className="text-xs text-slate-500 font-medium tracking-tight">{t.personaSub}</p></div>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5"><label className="text-[10px] font-black uppercase text-slate-400 ml-1">{t.agentName}</label><input value={settings.agentName} onChange={(e) => update('agentName', e.target.value)} className="w-full bg-slate-50 p-4 rounded-2xl text-sm font-bold" /></div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black uppercase text-slate-400 ml-1">{t.languageAgent}</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {[...MALE_VOICES, ...FEMALE_VOICES].map(v => (
                      <button key={v} onClick={() => update('voiceName', v)} className={`p-4 rounded-2xl border flex items-center justify-between ${settings.voiceName === v ? 'bg-teal-50 border-teal-500' : 'bg-slate-50 border-slate-100'}`}>
                        <span className="text-xs font-black uppercase">{v}</span>
          ...
          <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
            {step > 1 ? <button onClick={prev} className="flex items-center gap-2 text-slate-500 font-black text-xs uppercase"><ArrowLeft size={16} /> {t.back}</button> : <div />}
            <button onClick={step === 5 ? () => onFinish(settings) : next} className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center gap-3">{step === 5 ? <>{t.finish} <Check size={18} /></> : <>{t.continue} <ArrowRight size={18} /></>}</button>
          </div>
        </div>
      </div>
    </div>
